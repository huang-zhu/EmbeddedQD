...
ss_tot     = np.sum((ydata-np.mean(ydata))**2)
r_squared  = 1 - (ss_res / ss_tot)
ax_membraneFits[ax_membraneFits_index].plot(xdata, LL4(xdata, *popt), linewidth=1.5, color='blue', label='(2) R$^2$ = %.4f' %(r_squared))
...
ss_tot     = np.sum((ydata-np.mean(ydata))**2)
r_squared  = 1 - (ss_res / ss_tot)
ax_membraneFits[ax_membraneFits_index].plot(xdata, stretchedExp(xdata, *popt), linewidth=1.5, color='red', label='(3) R$^2$ = %.4f' %(r_squared))
...
ss_tot     = np.sum((ydata-np.mean(ydata))**2)
r_squared  = 1 - (ss_res / ss_tot)
plt.plot(xdata, stretchedExp(xdata, *popt), linewidth=1.5, color='red', label='(3) R$^2$ = %.4f' %(r_squared))
